...
center_wl=array([1163.002])#linspace(1130,1190,21)
...
chirp=linspace(0,30000,11)
omega_0=arange(1.04107,1.09107,0.002)#arange(1.06107,1.07107,0.001)
...
for i in range(len(omega_0)):
    config['qdot']['omega_yo']=omega_0[i]
...
#    config['pulse']['detun']=round(abs(round(center_omega[i],5)-dot_reson),5)
    ...
    for j in range(len(chirp)):
        config['masks'][mask_name]['phi_2']=chirp[j]
        config.write()
        print "Run parameters: omega_o="+str(center_omega)+" mask="+mask_name+" chirp="+str(chirp[j])+" omega_0= "+str(omega_0[i])
