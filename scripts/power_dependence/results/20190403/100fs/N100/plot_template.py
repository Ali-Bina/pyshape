...
k_values=np.zeros(len(data_files))
pulse=[]
...
for i in range(len(data_files)):
    ...
    wl[i]=float(data_files[i].rsplit('_')[2].rstrip("nm"))
    chirp[i]=float(data_files[i].rsplit('_')[1].rstrip("fs2"))
    k_values[i]=float(data_files[i].rsplit('_')[3].strip("k.txt"))
...
#    plt.plot(var[:,0],var[:,1],label="XX") #add 3rd variable 
    plt.plot(var[:,0],0.5*(var[:,2]+1),label="d_e/d_h ="+str(k_values[i]),linestyle=style[i],color=colour[i])
