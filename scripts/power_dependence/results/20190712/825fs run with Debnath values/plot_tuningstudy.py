...
chirp_files_p=glob.glob("occupation_6170000.0fs2*")
chirp_files_n=glob.glob("occupation_-6170000.0fs2*")
...
chirp_data_p=np.zeros([15,len(chirp_files_p)])
chirp_data_n=np.zeros([15,len(chirp_files_n)])
...
for j in range(6):
    plt.subplot(6,1,j+1)
    plt.plot(pulse_area,chirp_data_p[:,6+j],label="P300")
    plt.plot(pulse_area,chirp_data_n[:,6+j],label="N300")
    plt.ylim(0,1.1)
    plt.locator_params(nbins=4)
    plt.text(0,0.9,wl[6+j],fontsize=8)
    plt.legend(loc=4,frameon=False, prop={'size': 8})
    plt.tight_layout()
plt.show()
plt.savefig("chirp_compare2.svg")
