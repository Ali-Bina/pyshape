...
chirp_data=np.zeros([40,len(chirp_files)])
...
for j in range(15):
    plt.subplot(5,3,j+1)
    plt.plot(pulse_area,chirp_data[:,j],label='N300',linewidth=2,linestyle='--',color='r')
    plt.ylim(0,1.1)
    plt.locator_params(nbins=4)
    plt.text(2.0,0.2,str(round(float(omega_c[j])*1000,5))+' meV',fontsize=8)
