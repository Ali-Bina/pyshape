...
chirp_data=np.zeros([60,len(chirp_files)])
...
for j in range(9):
    plt.subplot(3,3,j+1)
    plt.plot(pulse_area,chirp_data[:,j],label='N300',linewidth=2,color='g')
    plt.ylim(0,1.1)
    plt.locator_params(nbins=4)
    plt.text(2.0,0.2,str(float(omega_c[j])*1000)+" meV",fontsize=8)
